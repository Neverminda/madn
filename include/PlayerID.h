 ...
 */
inline char to_char(PlayerID id) {
 ...
 */
inline PlayerID to_player_id(int value) {
    return static_cast<PlayerID>(value);
}
 ...
 */
inline PlayerID next_player(PlayerID current) {
    return static_cast<PlayerID>((static_cast<int>(current) + 1) % 4);
}
