...
void player_task(Game& game, int player_id) {
    ...
    while (true) {
        ...
        if (!possible_move_pawns.empty()) {
            ...
            stats.save(player_id, roll, pawn_to_move_index, game.pawn_positions);
        }
        else {
            stats.save(player_id, roll, -1, game.pawn_positions);
        }
        // else: Kein Zug möglich (z.B. alles blockiert), Zug verfällt.

        // 3. Spielzustand ausgeben (während Lock gehalten wird!)
        if constexpr (false) game.print_game_state(roll);
...
int main()
{
    ...
    std::println("total_moves {}", stats.moves);
    std::println("moves_per_game {:.4}", (double)stats.moves/runs);
    std::println("storage_size {}byte", stats.moves * stats.state_size);
}